...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function setupExpandingCirclesPreloader() {
    ...
    function animate(timestamp) {
      ...
      ctx.fillStyle = "rgba(116, 87, 233, 0.26)";
      ctx.fill();
      for (let c = 0; c < circleCount; c++) {
        ...
        ctx.strokeStyle = `rgba(255, 158, 61, ${opacity * 0.2})`;
        ctx.lineWidth = 1;
        ctx.stroke();
        for (let i = 0; i < dotCount; i++) {
          ...
          ctx.lineTo(x, y);
          ctx.strokeStyle = `rgba(255, 158, 61, ${opacity * 0.1})`;
          ctx.lineWidth = 1;
          ctx.stroke();
          ctx.beginPath();
          ctx.arc(x, y, size, 0, Math.PI * 2);
          ctx.fillStyle = `rgba(255, 158, 61, ${opacity * 0.9})`;
          ctx.fill();
        }
      }
  ...
  function initFloatingParticles() {
    ...
    for (let i = 0; i < numParticles; i++) {
      ...
      particle.style.backgroundColor = `rgba(255, ${
        Math.floor(Math.random() * 100) + 158
      }, ${Math.floor(Math.random() * 100) + 61}, ${
        Math.random() * 0.5 + 0.2
      })`;
  ...
  function drawCircularVisualizer() {
    ...
    // background tint to match orb color theme
    circularCtx.fillStyle = "rgba(50, 94, 110, 0.06)";
    circularCtx.fill();
    const numRings = 3;
    // custom ring colors: inner -> #c2c2c2, middle -> #707070, outer -> #333333
    const ringRGB = [
      { r: 0xc2, g: 0xc2, b: 0xc2 },
      { r: 0x70, g: 0x70, b: 0x70 },
      { r: 0x33, g: 0x33, b: 0x33 }
    ];
    for (let ring = 0; ring < numRings; ring++) {
      const ringRadius = baseRadius * (0.7 + ring * 0.15);
      const baseOpacity = 0.8 - ring * 0.2; // original
      const opacity = baseOpacity * 0.7; // ~30% reduction
      circularCtx.beginPath();
      ...
      const c = ringRGB[ring];
      circularCtx.strokeStyle = `rgba(${c.r}, ${c.g}, ${c.b}, ${opacity})`;
      // soften stroke thickness for a more minimal look
      circularCtx.lineWidth = 1 + (numRings - ring) * 0.6;
      circularCtx.stroke();
      circularCtx.shadowBlur = 15;
      circularCtx.shadowColor = `rgba(${c.r}, ${c.g}, ${c.b}, ${opacity * 0.6})`;
    }
  ...
  function drawSpectrumAnalyzer() {
    ...
    spectrumCtx.strokeStyle = "rgba(61, 109, 255, 0.2)";
    ...
    spectrumCtx.fillStyle = "rgba(61, 194, 255, 0.7)";
    spectrumCtx.font = '10px "Red Hat Mono", sans-serif';
  ...
  function updateAudioWave() {
    ...
    wave.style.borderColor = `rgba(255, 158, 61, ${
  ...
  function calculateAudioMetrics() {
    ...
    if (stabilityValue < 40) {
      ...
      document.getElementById("status-indicator").style.color = "#062f2cff";
    } else {
      document.getElementById("status-indicator").style.color = "#2c413fff";
    }
    if (Math.random() < 0.05) {
  ...
  function drawWaveform() {
    ...
    if (audioAnalyser) {
      ...
      waveformCtx.strokeStyle = "rgba(255, 158, 61, 0.8)";
  ...
  function initThreeJS() {
    ...
    scene.fog = new THREE.FogExp2(0x0f181f, 0.05);
    ...
    const pointLight1 = new THREE.PointLight(0xff9e3d, 1, 10);
    pointLight1.position.set(2, 2, 2);
    scene.add(pointLight1);
    const pointLight2 = new THREE.PointLight(0x5f666d, 1, 10);
  ...
  function createBackgroundParticles() {
    ...
    const color1 = new THREE.Color(0xff9e3d);
    const color2 = new THREE.Color(0x5f666d);
    const color3 = new THREE.Color(0xb7bbbe);
  ...
  function createAnomalyObject() {
    ...
    // 1) Inner emissive core (bright, pulses with audio)
    const coreGeometry = new THREE.SphereGeometry(radius * 0.6, 48, 48);
    const coreMaterial = new THREE.ShaderMaterial({
      uniforms: {
        time: { value: 0 },
        audioLevel: { value: 0 },
        colorA: { value: new THREE.Color(0x325e6e) }, // rgb(50,94,110)
        colorB: { value: new THREE.Color(0x4c8aa0) }  // lighter teal blend
      },
      vertexShader: `
        ...
        void main() {
          ...
          // subtle breathing based on audio
          float scale = 1.0 + audioLevel * 0.05;
      ...
      fragmentShader: `
        ...
        varying vec3 vNormal;
        void main(){
          // center-bright look using inverse fresnel
          vec3 viewDir = normalize(cameraPosition - vPos);
          float fresnel = 1.0 - max(0.0, dot(viewDir, vNormal));
          float centerGlow = pow(1.0 - fresnel, 2.0);
          float pulse = 0.65 + 0.35 * sin(time * 2.0 + audioLevel * 3.14159);
          vec3 col = mix(colorA, colorB, 0.35 + 0.35 * sin(time * 0.7));
          col *= (1.2 + audioLevel * 1.2) * (centerGlow + 0.35 * pulse);
          float alpha = clamp(0.6 + audioLevel * 0.3, 0.0, 1.0);
    ...
    anomalyObject.add(core);

    // 2) Orb surface (smooth, translucent, reacts to audio + noise)
    ...
    const surfaceMaterial = new THREE.ShaderMaterial({
      uniforms: {
        ...
        colorInner: { value: new THREE.Color(0x5f97ab) },
        colorOuter: { value: new THREE.Color(0x325e6e) }
      },
      vertexShader: `
        uniform float time;
        uniform float audioLevel;
        ...
        void main(){
          ...
          vec3 p = position;
          float t = time * 0.25;
          float nval = snoise(n * 1.2 + vec3(t, t*0.7, t*1.3));
          float disp = (0.12 + 0.18 * audioLevel) * distortion * nval;
          p += n * disp;
          vPos = p;
          // world position/normal for lighting
      ...
      fragmentShader: `
        ...
        // Simple Blinn-Phong specular to give a light metallic sheen
        void main(){
          vec3 N = normalize(vWorldNormal);
          vec3 V = normalize(cameraPosition - vWorldPos);
          // fake key light from top-right
          ...
          float rim = pow(fres, 2.0 + audioLevel * 2.5);
          float pulse = 0.5 + 0.5 * sin(time * 2.0);
          vec3 base = mix(colorInner, colorOuter, 0.6 + 0.4 * pulse);
          
          // specular amount subtly modulated by audio (more shine with louder audio)
          float specPower = mix(32.0, 48.0, clamp(audioLevel, 0.0, 1.0));
          float spec = pow(ndoth, specPower) * (0.15 + audioLevel * 0.25);
          
          // final shading: base + small diffuse + rim + specular
          vec3 col = base * (0.35 + 0.45 * ndotl) * (1.0 + audioLevel * 0.4);
          col += rim * 0.25 * base;
          col += spec * vec3(0.95, 0.98, 1.0); // cool specular tint for metallic feel
          
          float alpha = 0.82 - audioLevel * 0.18; // slightly more transparent with louder audio
          gl_FragColor = vec4(col, clamp(alpha, 0.4, 0.92));
        }
      `,
      transparent: true,
      depthWrite: false
    ...
    const glowMaterial = new THREE.ShaderMaterial({
      uniforms: {
        ...
        color: {
          value: new THREE.Color(0x325e6e)
        },
        audioLevel: {
          value: 0
        }
    ...
    return function updateAnomaly(time, audioLevel) {
      ...
      coreMaterial.uniforms.audioLevel.value = audioLevel;
      // surface
      surfaceMaterial.uniforms.time.value = time;
      surfaceMaterial.uniforms.audioLevel.value = audioLevel;
      surfaceMaterial.uniforms.distortion.value = distortionAmount;
      // aura
  ...
  document.getElementById("analyze-btn").addEventListener("click", function () {
    ...
    if (statusEl) statusEl.style.color = "#ff00a0";
    setTimeout(() => {
  ...
  function makePanelDraggable(element, handle = null) {
    Draggable.create(element, {
      ...
      onDragStart: function () {
        // If the element is centered with a CSS transform, convert to absolute px
        const style = window.getComputedStyle(element);
        const hasTransform = style.transform && style.transform !== 'none';
        if (hasTransform) {
          const rect = element.getBoundingClientRect();
          // Lock current visual position as absolute pixels
          element.style.left = `${rect.left + window.scrollX}px`;
          element.style.top = `${rect.top + window.scrollY}px`;
          element.style.transform = 'none';
        }
        if (element.classList.contains('intro-panel')) {
          element.dataset.userMoved = '1';
        }
  ...
  // Make intro panel draggable
  const introPanelEl = document.querySelector('.intro-panel');
  if (introPanelEl) {
    makePanelDraggable(introPanelEl);
  }

  // Guide users to the spectrum analyzer from the intro panel
  const introListenBtn = document.getElementById('intro-listen-btn');
  if (introListenBtn) {
    introListenBtn.addEventListener('click', () => {
      const spec = document.querySelector('.spectrum-analyzer');
      if (!spec) return;
      spec.classList.add('highlighted');
      // Scroll into view if it's off-screen
      try { spec.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); } catch (e) {}
      setTimeout(() => spec.classList.remove('highlighted'), 1800);
    });
  }

  // Smart bump-away for intro panel to avoid overlapping anomaly/scanner
  function positionIntroPanel() {
    const intro = document.querySelector('.intro-panel');
    if (!intro || intro.dataset.userMoved === '1') return; // don't override user's drag

    const blockers = [
      document.querySelector('.scanner-frame'),
      document.querySelector('.circular-visualizer'),
      document.getElementById('audio-wave')
    ].filter(Boolean);

    if (blockers.length === 0) return;

    // Get the top boundary to avoid (minimum top among blockers)
    let minTop = Infinity;
    blockers.forEach((el) => {
    ...
    // If overlapping, move intro up so its bottom sits margin-above the top boundary
    if (introRect.bottom > (minTop - margin)) {
      const targetTop = Math.max(8, Math.round(minTop - introRect.height - margin));
      // Keep it horizontally centered via transform; adjust only top
      intro.style.top = `${targetTop}px`;
    }
  }

  // Run after layout settles and on resize
  requestAnimationFrame(() => {
    setTimeout(positionIntroPanel, 0);
  });
  window.addEventListener('resize', () => {
    // small debounce
    clearTimeout(window.__introPosTO);
    window.__introPosTO = setTimeout(positionIntroPanel, 100);
  });
});